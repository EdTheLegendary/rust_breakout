...
pub fn draw_title_text(text: &str, font: Font) {
    let dims = measure_text(text, Some(font), 40u16, 1.0f32);
    draw_text_ex(
        text,
        screen_width() * 0.5f32 - dims.width * 0.5f32,
        screen_height() * 0.5f32 - dims.width * 0.5f32,
...
async fn main() {
    ...
    balls.push(Ball::new(vec2(screen_width() * 0.47f32, screen_height() * 0.75f32)));
    ...
    loop {
        ...
        match game_state {
            GameState::Menu => {
                draw_title_text("Press SPACE", font);
            },
            GameState::Game => {
